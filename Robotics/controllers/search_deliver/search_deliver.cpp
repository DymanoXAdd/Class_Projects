...
CFootBotSearchDeliver::SFoodData::SFoodData() :
   HasFoodItem(false),
   FoodItemIdx(0),
   TotalFoodItems(0) {}

void CFootBotSearchDeliver::SFoodData::Reset() {
   HasFoodItem = false;
   FoodItemIdx = 0;
   TotalFoodItems = 0;
}
...
CFootBotSearchDeliver::SDiffusionParams::SDiffusionParams() :
   GoStraightAngleRange(CRadians(-1.0f), CRadians(1.0f)) {}

void CFootBotSearchDeliver::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
...
void CFootBotSearchDeliver::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
...
CFootBotSearchDeliver::SStateData::SStateData() :
   ProbRange(0.0f, 1.0f) {}

void CFootBotSearchDeliver::SStateData::Init(TConfigurationNode& t_node) {
   try {
...
void CFootBotSearchDeliver::SStateData::Reset() {
   State = STATE_RESTING;
   InNest = true;
...
CFootBotSearchDeliver::CFootBotSearchDeliver() :
   m_pcWheels(NULL),
   m_pcLEDs(NULL),
   m_pcRABA(NULL),
   m_pcRABS(NULL),
...
void CFootBotSearchDeliver::Init(TConfigurationNode& t_node) {
   try {
      /*
      ...
      THROW_ARGOSEXCEPTION_NESTED("Error initializing the foot-bot SearchDeliver controller for robot \"" << GetId() << "\"", ex);
   }
   /*
...
void CFootBotSearchDeliver::ControlStep() {
...
void CFootBotSearchDeliver::Reset() {
   /* Reset robot state */
   m_sStateData.Reset();
   /* Reset food data */
   m_sFoodData.Reset();
...
void CFootBotSearchDeliver::UpdateState() {
   /* Reset state flags */
...
CVector2 CFootBotSearchDeliver::CalculateVectorToLight() {
...
CVector2 CFootBotSearchDeliver::DiffusionVector(bool& b_collision) {
...
void CFootBotSearchDeliver::SetWheelSpeedsFromVector(const CVector2& c_heading) {
...
void CFootBotSearchDeliver::Rest() {
   ...
   if(m_sStateData.TimeRested > m_sStateData.MinimumRestingTime &&
      ...
      if(m_sStateData.TimeRested == 1) {
         m_pcRABA->SetData(0, LAST_EXPLORATION_NONE);
         //LOG<<"LAST_EXPLORATION_NONE ****"<< std::endl;
      }
      /*
...
void CFootBotSearchDeliver::Explore() {
    ...
    * nest' if we have been wandering for some time and found nothing */
      ...
      if (m_pcRNG->Uniform(m_sStateData.ProbRange) < m_sStateData.ExploreToRestProb) {
         ...
         LOG<<"***Unsuccessful... Keep Searching***"<<std::endl;
         /* Switch to 'return to nest' */
         bReturnToNest = false;
      }
      else {
...
void CFootBotSearchDeliver::ReturnToNest() {
 ...
 */
REGISTER_CONTROLLER(CFootBotSearchDeliver, "search_deliver_controller")
